...
struct HookInstaller {
    ...
    private static func updateSettings(at settingsURL: URL) {
        ...
        // Strip any existing CcIslandCn hooks from ALL event types first — even
        ...
        for (event, value) in hooks {
            if let entries = value as? [[String: Any]] {
                let cleaned = entries.compactMap { removingCcIslandCnHooks(from: $0) }
                if !cleaned.isEmpty {
    ...
    static func uninstall() {
        ...
        for (event, value) in hooks {
            if var entries = value as? [[String: Any]] {
                entries = entries.compactMap { removingCcIslandCnHooks(from: $0) }
    ...
    nonisolated private static func removingCcIslandCnHooks(from entry: [String: Any]) -> [String: Any]? {
        guard var entryHooks = entry["hooks"] as? [[String: Any]] else {
            return entry
        }

        entryHooks.removeAll(where: isCcIslandCnHook)
    ...
    nonisolated private static func isCcIslandCnHook(_ hook: [String: Any]) -> Bool {
